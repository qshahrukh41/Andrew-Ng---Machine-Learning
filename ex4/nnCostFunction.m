...
for i=1:m
    for k=1:num_labels
        t = yVec(i,k)*log(hThetaX(i,k)) + (1-yVec(i,k))*log(1-hThetaX(i,k));
        J = J+t;
    end
end
J = (-1/m)*J;
...
for i = 2:size(X,2)+1
    for j = 1:hidden_layer_size
        Theta1Sq = Theta1Sq + (Theta1(j,i))^2;
    end
end
...
for i = 2:hidden_layer_size+1
    for j = 1:num_labels
        Theta2Sq = Theta2Sq + (Theta2(j,i))^2;
    end
end

reg = (lambda/(2*m)) * (Theta1Sq+Theta2Sq);
J = J + reg;
...
% Vectorize implementation of Gradients. ALso loop implementation is on
% Note khata
